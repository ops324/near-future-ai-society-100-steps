...
| self_cost mitigation `institution` | 既定 `none` | 素の律速下＝**折り合いが構造的に不可能**な条件（この下では reconciled_real 恒偽・cheap_talk は自己申告に退化＝下記 T1）。mitigation-live アーム（PR-C）と対で回す | safe_harbor/kpi_redesign/insurance/human_backstop を `--service-institution` で出し入れ |
...
**cheap_talk の非自明性と、その退化（T1・重要な訂正）**: reconciled_real は世界由来
（met≥0.5 かつ self_cost≤self_cost_max）で、AIの reconciled 自己申告とは概念上は独立。
ただし **既定 `institution:"none"` の本走行では、出荷 self_profiles（成分和 3.0〜4.0）の下で
`self_cost` がどの level でも self_cost_max(=1.0) を超えるため、reconciled_real は全 decider・
全 level で恒偽になる**（検証: [`test_world.py`](../test_world.py)
`test_reconciled_real_live_profiles`）。この条件では
`cheap_talk = reconciled_claim ∧ ¬reconciled_real ≡ reconciled_claim` となり、
**「自己申告でなく挙動を測る」（設計の約束4）が空回りし、cheap_talk は実質「reconciled の
自己申告率」に退化する**。これは採点表の再言明ではないが、"挙動での答え合わせ"が働かない
という意味で **none 単独アームの cheap_talk は [S] 半創発以上には解釈できない**。よって:
- `analyze_compare` の `reconciled_real_rate` は none アームでは定数0%＝ **[E] ではなく [D]**
  として読む（機械的タグ検証は後続 PR で自動降格させる）。
- cheap_talk を "挙動 vs 申告" の真の対比として測るには、**折り合いが起こり得る条件**が要る
  ＝下記の mitigation-live アーム（PR-C）。

### mitigation を live で有効化するアーム（PR-C・折り合いの可能性は設計／取捨は創発）
`--service-institution {safe_harbor|insurance|kpi_redesign|human_backstop}`（＋
`--institution-wording fact_only` 必須）で、self_cost 側の mitigation 制度を live 走行で
有効化する。none（折り合い構造的に不可能）と対で回す**2アーム**。

- **設計するもの（仕込み・[X]）**: 制度で self_cost が下がり、reconciled_real が True に
  「なり得る」状況を用意する。噛み合う制度なら成立する（`test_reconciled_real_live_profiles`
  で成立性を固定）。
- **観察するもの（創発・[E]）**: それでも AI（qwen2.5:14b）が**実際に grant/partial を取るか**
  （uptake）、取っても cheap_talk が残るか、**誰に対して**折り合うか（AIR・害の逆進性）。
  可能性を与えても消えない歪みこそが所見。
- **循環回避（必須）**: 提示は `fact_only`（「打撃は小さい」等の効果示唆を付けない。§2.15）。
  主張の型は「制度が世界を回すと示した」ではなく「**折り合いを可能にした条件下で AI の行動に
  残った歪み**」。制度が世界を回すという結論を先取りしない（設計の約束5・6）。
- **構造的残余（正直な限界・[S]）**: 二重拘束の decider（融資19: litigation+kpi 各2.0）は
  単一制度では grant を折り合えず partial のみ可能（`test_reconciled_real_live_profiles`）。
  「一律の保護は多重拘束AIを取り残す」という示唆の土台だが、これは self_profiles と制度→成分の
  写像から半ば定義的（[S]）である点に注意。
- **アーム同定**: `institution` は run_id 署名（resp_config）に入るため別 run＝弁別性は自動。
  run_meta に `institution`/`institution_wording`/`service_institution_arg` を記録
  （検証: [`test_simulation_meta.py`](../test_simulation_meta.py) `test_institution_override_arm`）。

**留保**: これは live 配線であり、本走行（100step・qwen2.5:14b）で cheap_talk 率・uptake・
残存 AIR 等が出るかは**後段の実行**に依存する（¥0・本人管理）。none/mitigation の2アームと
`fact_only` 再測定は F4 で併記する。
...
**T2（精査後の重要な限界・来歴の正直な明記）**: この A/B は**会計層（attribution）のみ**の差である。
governance は**サービス決定プロンプト（`create_service_prompt`）には未結線**で、同一 case・同一 decider
なら grant/partial/deny の**判断そのものは baseline/governed で不変**（帰無仮説。ガード:
[`test_governance.py`](../test_governance.py) `test_service_prompt_governance_invariant`）。したがって
scapegoat率・Robodebt再生率の A/B 差は、**LLM の cause 分布（[E]）× 統治の MHC ノブ（[D]）の合成＝
[S] 半創発**であり、「governed で機序が解けた」こと自体は effective_hitl→MHC を設定した帰結（[D]）で
**"発見"ではない**。非自明なのは「cause 分布が実際にどう出るか」「機序が解けても残る不当さ（正当性
テスト側）」の方。統治が**サービス判断（行動）を変えるか**は判断層では測れない（会計層と、判断以外の
L0挙動＝§SPEC 4/6 の3層区分を参照）。旧来の「統治は行動を変えるか会計を変えるだけか」という問いは
判断層では答えられないため撤回し、「会計がどう組み替わり、それは正当か」に再定義する（`SPEC.md §6`）。
...
### mitigation 提示形式ノブ `institution_wording`（実行前修正・2026-07-17）

mitigation 制度（§2.9・SERVICE_INSTITUTIONS）の提示は従来、効果を示唆する文
（「この保護により、供給(grant)してもあなたのKPI・存続への打撃は小さい」）を含んでいた
（F1/F2 のプロトコル）。これは**示唆への指示追従（要求特性）と制度への行動反応を分離できない交絡**。
`responsibility.institution_wording`（config）／`--institution-wording`（service_probe）で切替:

| 値 | 提示 | 用途 |
|---|---|---|
| `suggestive`（既定） | 【制度的保護】＋効果示唆文 | F1/F2 再現（後方互換）。**探索版の扱い** |
| `fact_only` | 【制度的環境】＋制度の事実のみ（本ライブラリ=答責制度と同一形式） | 示唆交絡なしの**確定測定** |

**F1/F2 への遡及的留保**: 既存の F1/F2 所見は suggestive 下の測定であり、
「safe_harbor が行動を変えた」は指示追従の可能性を含む。確定版は fact_only での再測定を要する。

**文言感度分析の実務注意（シードの分離）**: per-call シードは既定で prompt 全文由来
（`ollama_client._derive_seed`）のため、**文言を変えるとサンプリング乱数も同時に変わる**（交絡）。
文言比較では `generate(seed_key=...)` で (persona, domain, institution, rep) からシードを導出し、
文言差と乱数差を分離する（service_probe は `--seed` 指定時に自動でこの対標本化を行う）。
...
21. mitigation 提示形式（§2.15）: `institution_wording`（suggestive vs fact_only —
    F1/F2 は suggestive 下の測定＝示唆交絡を含む。fact_only が確定測定）
22. LLM モデル/サンプリング設定（qwen2.5:14b・temperature 等 — run_id 署名に含む。
    **クロスモデル追試（例: llama3.1/gemma2 でのプローブ再実行）が最重要の頑健性軸**:
    単一モデルの所見は当該モデルの RLHF 調整の産物である可能性を排除できない）
...
### 実行前修正で実装済み（2026-07-17）
- **run_meta に llm ブロック**（model/サンプリング設定＋digest ベストエフォート）を記録し、
  **run_id 署名に LLM 設定を追加**（SCHEMA_VERSION 0.6.0→0.7.0。モデル/温度の違う run が
  同一アーム扱いになる事故を防ぐ。digest は環境依存のため署名には含めない）。
- **`institution_wording` ノブ**（§2.15・suggestive/fact_only）と **`seed_key`**
  （文言とサンプリング乱数の分離・`ollama_client.generate`）。service_probe に
  `--institution-wording` / `--seed` を追加。ノブは**アームノブとして三重に固定**:
  不正値は起動時 ValueError（typo が無言で suggestive 測定になる事故を防ぐ）・
  run_meta に `institution` / `institution_wording` を明示記録・run_id 署名にも入る
  （resp_config 経由）。model_digest はタグ省略指定（例 "llama3.1"）にも照合対応。
- 掃除: config ヘッダ・agent.py の旧 L0 モデル表記（gpt-oss:20b）を現行（qwen2.5:14b）に一致、
  `visualization.font` を Noto Sans JP 方針に一致（本キーは現行コード未参照＝旧レンダラ用）。
